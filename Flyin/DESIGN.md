...
| Port | 外界とやり取りする抽象インターフェース（ABC で統一） |
| Adapter | Port の具象実装 |
...
**全 Port を ABC（`abc.ABC` + `@abstractmethod`）で統一する**（§19.5 で確定）。
PDF が「completely object-oriented」を要求し、peer review で構造を実演する必要があるため、
構造的型付け（`Protocol`）でなく **明示的継承** を採用し、クラス階層を可視化する。
mypy は未実装の抽象メソッドを検出でき、実装漏れを型レベルで防げる。
...
**中核は `FlowPathPlanner`（最小費用流）に確定**（§19.3）。容量（`max_drones`/`max_link_capacity`）と
重み（`restricted`=2）を統一的に最適化でき、評価対象の容量系ベンチマーク（Basic capacity / Capacity hell）
を正しく解けるのはフローのみ、という PDF VII.7 の根拠による。

| Adapter | 内容 | 位置づけ |
|---------|------|----------|
| `DisjointPathPlanner` | 重み付き探索で素な経路を反復抽出＋lem-in 分配 | 開発初期の足場（正当性ブートストラップ／単純マップ比較用） |
| `FlowPathPlanner` | 容量・重みを最小費用流で同時最適化（頂点分割・時間展開） | **必須中核** |
| `AdaptivePlanner` | マップ規模・形状で内部戦略を選択 | ボーナス（全目標「完全達成」/Challenger 調整） |
...
def select_planner(args: CliArgs) -> PathPlanner:
    if args.planner == "disjoint":
        return DisjointPathPlanner()                                     # 単純マップ/比較用
    if args.planner == "adaptive":
        return AdaptivePlanner(DisjointPathPlanner(), FlowPathPlanner())  # 規模で選択（ボーナス）
    return FlowPathPlanner()                                             # 既定＝必須中核
```
...
- **必須はターミナル描画のみ**（§19.2）: PDF VII.1／Chapter X が "terminal colors **and/or** graphical
  interface" としており、色付きターミナルで必須要件を充足する。
- 具象: `TerminalRenderer`（ANSI カラー、ゾーン `color=`・種別・ドローン位置・凡例／必須）、
  `NullRenderer`（ヘッドレス・テスト用）、`GuiRenderer`（**ボーナス**・`Renderer` 実装で非改変追加）。
...
- **接続名は `<name1>-<name2>`**（§19.1）: 接続定義の端点順に `-` 連結。ゾーン名はダッシュ禁止（VII.4）
  なので `D1-corridorA-tunnelB` は先頭 `-` で `D1` と `corridorA-tunnelB` に一意分解できる。
  例: 接続 `corridorA-tunnelB` 経由で restricted `tunnelB` へ → t: `D1-corridorA-tunnelB`、t+1: `D1-tunnelB`。
...
│   ├── network.py           # Network, NetworkView(ABC)
...
| 項目 | 方針 |
|------|------|
| 型ヒント | 全関数引数・戻り値・変数。Port は ABC で契約を型として明示 |
...
flowchart LR
    ...
    B["2. ports<br/>抽象定義(ABC)"]
    C["3. parsing adapters<br/>Handler/Builder/Validator"]
    D["4. planning(足場)<br/>DisjointPathPlanner"]
    E["5. application<br/>Engine+State+Policy"]
    F["6. planning(中核)<br/>FlowPathPlanner 最小費用流/時間展開"]
    G["7. output/rendering<br/>Formatter+TerminalRenderer"]
    H["8. tests+benchmark"]
    I["9. AdaptivePlanner/Challenger 調整(ボーナス)"]
    A-->B-->C-->D-->E-->F-->G-->H-->I
```
...
旧「確認ポイント」を、PDF 仕様の記述から判断して確定する。

### 19.1 接続名の出力表記 →『`<name1>-<name2>`』
- **根拠**: VII.4「Zone names ... except dashes」「connection syntax forbids dashes in zone names」。
  ゾーン名にダッシュを禁じる目的は、接続識別子 `name1-name2` を一意にパースさせるため。
- **決定**: restricted 飛行中の `D<ID>-<connection>` の接続名は、接続定義の端点を定義順に `-` で連結した文字列。
  方向情報は不要（次ターンの `D<ID>-<zone>` で到達先が判明する）。

### 19.2 可視化の範囲 →『必須はターミナル描画のみ・GUI はボーナス』
- **根拠**: VII.1／Chapter X「terminal colors **and/or** graphical interface」。色付きターミナルで必須充足。
- **決定**: 必須は `TerminalRenderer`。`GuiRenderer` は `Renderer` 実装としていつでも非改変追加できるボーナス。

### 19.3 経路探索の中核 →『最小費用流（`FlowPathPlanner`）を必須中核に』
- **根拠**: 提出物（Chapter X）は「minimizing total turns」可能な探索を要求。VII.7 の評価対象ベンチマークに
  容量系（Basic capacity / Capacity hell）が含まれ、容量（`max_drones`/`max_link_capacity`）と重み
  （`restricted`=2）を統一最適化できるのは最小費用流。素な経路法は容量 >1 を最適に扱えない。
- **決定**: `FlowPathPlanner`（頂点分割＋時間展開）を必須中核に確定。`DisjointPathPlanner` は開発初期の
  正当性ブートストラップ／単純マップ比較用、`AdaptivePlanner` はボーナス調整に留める。

### 19.4 priority のタイブレーク →『コストは 1 のまま、同ターン解の二次目的で優先』
- **根拠**: VII.3「priority: 1 turn (but should be preferred in pathfinding algorithms)」。
  コストは normal と同じ 1。優先は探索上の選好であって、ターン数を増やしてはならない。
- **決定**: `CostModel.move_cost(priority)=1` を厳守し、`preference()` は **同一ターン数の解の中での二次目的**
  （priority 経由を優先）としてのみ作用させる。主目的（総ターン最小）を侵さない。

### 19.5 Port の実装方式 →『ABC で統一』
- **根拠**: Chapter V「completely object-oriented」、Chapter X「explain your code ... during peer review」。
  明示的継承の方がクラス階層を可視化・実演しやすく、mypy が抽象メソッド未実装を検出できる。
- **決定**: 全 Port を `abc.ABC` + `@abstractmethod` で定義（`typing.Protocol` は不採用）。
